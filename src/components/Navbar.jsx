        ...
        <div className="border-b border-black/10">
          <div className="page-shell flex items-center justify-between py-3 text-black">
            ...
            <div className="flex items-center gap-5 text-black/70">
              <button type="button" aria-label="Search" className="transition hover:text-black" onClick={handleSearchClick}>
                <FiSearch size={17} />
              </button>
              <button type="button" aria-label="Wishlist" className="relative transition hover:text-black" onClick={handleWishlistClick}>
                <FiHeart size={17} />
                {wishlist.length > 0 && (
              ...
              <button type="button" aria-label="Account" className="transition hover:text-black" onClick={handleAccountClick}>
                <FiUser size={17} />
              </button>
              <button type="button" aria-label="Shopping cart" className="relative transition hover:text-black" onClick={handleCartClick}>
          ...
          <ul className="page-shell flex flex-col items-center gap-3 py-3 text-[0.65rem] font-medium uppercase tracking-[0.22em] text-black/60 sm:flex-row sm:justify-center sm:gap-10 sm:py-3">
            {SUB_LINKS.map((link) => (
              <li key={link.path} className="group relative">
                <Link to={link.path} className="transition hover:text-black" onClick={() => setMenuOpen(false)}>
                  {link.label}
